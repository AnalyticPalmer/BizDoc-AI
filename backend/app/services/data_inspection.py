...
class DataInspectionService:
    ...
    async def inspect_file(
        ...
        filename = DataInspectionService._validate_filename(file)
        ...
        if len(content) > MAX_FILE_SIZE_BYTES:
            raise HTTPException(
                ...
                detail="The file is larger than the 25 MB upload limit.",
            )

        try:
            if extension == ".csv":
                dataframe = DataInspectionService._read_csv(content)
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    "Please confirm that the file is valid and not corrupted."
                ),
            ) from exc

        dataframe = DataInspectionService._prepare_dataframe(dataframe)
        ...
        return {
            ...
            "completeness_percent": (
                DataInspectionService._calculate_completeness(dataframe)
            ),
            "preview": DataInspectionService._build_preview(dataframe),
        }
    ...
    def _read_csv(content: bytes) -> pd.DataFrame:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="BizDoctor could not determine the CSV file encoding.",
        ) from last_error
    ...
    def _read_excel(
        ...
        if not available_sheets:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="No worksheets were found in the Excel file.",
            )
        ...
        return dataframe, selected_sheet, available_sheets
    ...
    def _prepare_dataframe(
        ...
        if dataframe.empty and len(dataframe.columns) == 0:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="The uploaded dataset does not contain any data.",
            )

        dataframe.columns = [
            DataInspectionService._clean_column_name(column)
            for column in dataframe.columns
        ]

        duplicated_columns = dataframe.columns.duplicated()

        if duplicated_columns.any():
            dataframe.columns = (
                DataInspectionService._make_columns_unique(
    ...
    def _build_preview(
        ...
        for _, row in preview_dataframe.iterrows():
            ...
            for column, value in row.items():
                record[column] = (
                    DataInspectionService._make_json_safe(
                        value
                    )
                )
